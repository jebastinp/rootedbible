...
def list_plan(
    ...
    _: User = Depends(require_leader_up),
):
...
def create_plan_day(payload: ReadingPlanCreate, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    return ReadingPlanService(db).create(payload)


@router.patch("/{plan_id}", response_model=ReadingPlanOut, summary="Edit a reading day")
def update_plan_day(plan_id: uuid.UUID, payload: ReadingPlanUpdate, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    return ReadingPlanService(db).update(plan_id, payload)


@router.delete("/{plan_id}", summary="Delete a reading day")
def delete_plan_day(plan_id: uuid.UUID, db: Session = Depends(get_db), _: User = Depends(require_admin)):
