...
def create_member(payload: UserCreate, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    return UserService(db).create(payload)


@router.get("/{member_id}", response_model=UserOut, summary="Get a single member")
def get_member(member_id: uuid.UUID, db: Session = Depends(get_db), _: User = Depends(require_leader_up)):
