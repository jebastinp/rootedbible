...
def update_question(question_id: uuid.UUID, payload: QuizQuestionUpdate, current_user: User = Depends(require_super_admin), db: Session = Depends(get_db)):
...
def delete_question(question_id: uuid.UUID, current_user: User = Depends(require_super_admin), db: Session = Depends(get_db)):
