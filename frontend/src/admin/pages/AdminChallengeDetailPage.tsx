...
import type { ChallengeAdmin, ChallengeReward } from '@/types'
...
const TABS = ['Overview', 'Members', 'Requests', 'Families', 'Buddy Groups', 'Rewards'] as const
...
export default function AdminChallengeDetailPage() {
  ...
  return (
    <div>
      <AdminPageHeader title={challenge.name} description={`${challenge.church_name} · ${challenge.participant_count} / ${challenge.participant_limit} participants`} />
      ...
      <div className="p-8 pt-5">
        {tab === 'Overview' && (
          <div className="grid grid-cols-3 gap-4">
            <StatCard label="Participants" value={`${challenge.participant_count} / ${challenge.participant_limit}`} />
            <StatCard label="Status" value={challenge.status} />
            <StatCard label="Families" value={challenge.allow_families ? `max ${challenge.family_limit}` : 'disabled'} />
            <StatCard label="Buddy Groups" value={challenge.allow_buddies ? `max ${challenge.buddy_limit}` : 'disabled'} />
        ...
        {tab === 'Requests' && (
          <div className="bg-surface rounded-2xl shadow-soft divide-y divide-ink/5 overflow-hidden">
            {!requests?.length ? (
...
function GroupList({ groups, empty }: { groups?: GroupDetailAdmin[]; empty: string }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      {groups.map((g) => (
        <div key={g.id} className="bg-surface rounded-2xl p-5 shadow-soft">
...
function CreateRewardModal({ challengeId, onClose }: { challengeId: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-sm bg-surface rounded-2xl shadow-card p-6 space-y-4">
